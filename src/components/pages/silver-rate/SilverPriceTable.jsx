...
const PURITIES = [999, 958, 925];
const WEIGHTS = [1, 10, 100, 1000];

export default function SilverPriceTable({ baseRate }) {
    const perGramRate = baseRate / 10;

    return (
        <section id="silver-price-breakdown-weight" className="silver-price-breakdown-weight py-8 md:py-10 bg-white border-t border-zinc-50 relative overflow-hidden">
                                ...
                                <tr className="bg-zinc-50/80 backdrop-blur-sm">
                                    ...
                                    {WEIGHTS.map(w => (
                                        <th key={w} className="p-2 md:p-4 text-[10px] md:text-[13px] font-bold text-zinc-400 uppercase tracking-[0.2em] border-b border-l border-zinc-100 text-center font-figtree whitespace-nowrap">
                                            {w >= 1000 ? `${w / 1000} KG` : w >= 100 ? `${w} GM` : `${w} GM`}
                                        </th>
                                    ))}
                                </tr>
                            </thead>
                                        ...
                                        <tr key={purity} className="hover:bg-[#FAF3EC]/40 transition-colors group">
                                            <td className="p-2 md:p-4 border-r border-zinc-100">
                                                <div className="flex items-center gap-2 md:gap-4">
                                                    <div className={`w-4 h-4 md:w-10 md:h-10 rounded-full flex items-center justify-center text-[10px] md:text-[12px] font-bold font-figtree shrink-0 ${purity >= 999 ? 'bg-primary text-white' : 'bg-zinc-100 text-zinc-500'}`}>
                                                        {purity}
                                                    </div>
                                                    <span className="text-[14px] md:text-[20px] font-bold text-zinc-900 font-figtree whitespace-nowrap">{purity} Silver</span>
                                                </div>
                                            </td>
                                            {WEIGHTS.map(w => {
