...
const PURITIES = [999, 950, 900];
const WEIGHTS = [1, 5, 10, 100];

export default function PlatinumPriceTable({ baseRate }) {
    const perGramRate = baseRate / 10;

    return (
        <section id="platinum-price-breakdown-weight" className="platinum-price-breakdown-weight py-8 md:py-10 bg-white border-t border-zinc-50 relative overflow-hidden">
            ...
            <div className="container-main relative z-10">
                <div className="max-w-4xl mx-auto text-center mb-16 space-y-4">
                    ...
                    <p className="text-zinc-400 font-figtree text-[14px] md:text-[18px] max-w-2xl mx-auto px-4">
                        Detailed pricing overview across different platinum purities and weights, including the latest market updates.
                    </p>
                </div>
                        ...
                        <table className="w-full border-collapse">
                            ...
                            <tbody className="divide-y divide-zinc-50">
                                {PURITIES.map(purity => {
                                    ...
                                    return (
                                        <tr key={purity} className="hover:bg-[#FAF3EC]/40 transition-colors group">
                                            <td className="p-2 md:p-4 border-r border-zinc-100">
                                                <div className="flex items-center gap-2 md:gap-4">
                                                    <div className={`w-4 h-4 md:w-10 md:h-10 rounded-full flex items-center justify-center text-[10px] md:text-[12px] font-bold font-figtree shrink-0 ${purity >= 950 ? 'bg-primary text-white' : 'bg-zinc-100 text-zinc-500'}`}>
                                                        {purity}
                                                    </div>
                                                    <span className="text-[14px] md:text-[20px] font-bold text-zinc-900 font-figtree whitespace-nowrap">Pt{purity}</span>
                                                </div>
                                            </td>
                                            {WEIGHTS.map(w => {
                                                const price = perGramRate * w * purityFactor;
                                                return (
