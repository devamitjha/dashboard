...
export default function StyledVideosCollectionDashboard() {
  ...
  return (
    <div className='max-w-7xl mx-auto py-10 px-8'>
      <div className='flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6'>
        <div>
          <h1 className='text-3xl font-bold text-zinc-900 font-figtree'>Styled Video Collection Specific</h1>
          <p className='text-zinc-500 mt-1'>Manage collection-specific videos and tagged products.</p>
        </div>
                ...
                <div className='flex-1 space-y-8'>
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                    <div className='space-y-3'>
                      ...
                      <input value={video.video || ''} onChange={e => updateVideoUrl(vIndex, e.target.value)} className='w-full px-5 py-3.5 bg-zinc-50 border border-zinc-100 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-black' placeholder='Paste Shopify CDN URL here...' />
                    </div>
                    <div className='space-y-3'>
                      <label className='text-[10px] font-black uppercase tracking-widest text-zinc-400 flex items-center gap-2'><Layers size={12} /> COLLECTION HANDLE</label>
                      <input value={video.collectionHandle || ''} onChange={e => updateCollectionHandle(vIndex, e.target.value)} className='w-full px-5 py-3.5 bg-zinc-50 border border-zinc-100 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-black' placeholder='e.g. hexa, rings, etc.' />
                    </div>
                  </div>
  ...
  nv[vIndex] = video;
  setVideos(nv); 
}} className='p-1.5 hover:bg-rose-50 text-rose-300 hover:text-rose-500 rounded-lg'><Trash2 size={14} /></button>
                          </div>
                        </div>
                      ))}
